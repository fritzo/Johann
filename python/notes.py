...
"""
    (Q1) how to decide whether to surmise the existence using an axiom,
      or to assert the existence?
      (N1) every new ob $x$ needs (is) the application $I*x=x$, thus making a
        new application app=lhs*rhs requires the application I*app=app.  this
        can still be done MCMC-style if the combined probability (of both new
        apps) is used.
      (N2) to find pure app creations, one must randomly enforce axiom scheme
        instances.  thus one needs to define a measure on axiom scheme
        instances.  define
        (??) app = lhs * rhs
          P = p(app) + (1-eps) * p(lhs) * p(rhs)
        (K1) x = Kx*y
        (S1) xz_yz = Sxy*z
        (S2) Sxyz = xz*yz
      (N3) there also needs to be a measure on nodes rather than obs.  define
        (V1) P(node) = node.app.splitEps * node.lhs.joinEps * node.rhs.joinEps
          or something, so that split influence comes from the app, and join
          influence comes from the lhs & rhs.
          (N1) thus,  P_ob(x) = sum_{nodes x=y*z} P_app(x=y*z)
      (N4) MCMC-style random-walking requires an initiation period.  this is
        like settling on a "current price" for memory resources.  if apps are
        randomly created/deleted WRT the measure
          P(x) = meas(x)/resources(x)
        or, to include pruing,
          -log(P(x)) = delta_m/delta_n + const.,
        where delta_m is the measure difference and delta_n is the node
        difference.  for pruning, delta_n is always strictly negative, so the
        probability is well defined.  for creation, delta_n may be
        nonpositive, in which case the ob is decisively created.
        the "current price" is an approximation to the normalization factor
        in the node measure.
      (N5) note that delta_m is always underestimated, possibly causing an 
        MCMC jump bias.
      (A1) if nodes are created by the join measure and deleted by the inverse
        split measure, then the product distribution will be reached.  however,
        as the split measure changes rapidly whereas the join measure is static,
        it would make more sense to create by the split measure and delete by the
        inverse join measure.  this may be impossible.
...
def updateMeasure8_join(db,tol=1e-4):
 #define data representation
  MIO_num2root = list(db.MIO_rootIter())
...
(N7) the enforcement methods need to be updated to random creation.
  (N1) the difference now is that mergers are performed only if all
    equations exists:
  (P1) [K]: {K*x=Kx, Kx*y=Kxy}
    (K1) K*x=Kx | Kx*y=Kxy
      --> merge Kxy with x
    (K2) Kx*y=Kxy | K*x=Kx
      --> insert Kxy in x tree
  (P2) [S]: {S*x=Sx, Sx*y=Sxy, Sxy*z=Sxyz, x*z=xz, y*z=yz, xz*yz=xz_yz}
    (S1) S*x=Sx | Sx*y=Sxy, Sxy*z=Sxyz, x*z=xz, y*z=yz, xz*yz=xz_yz
      --> merge Sxyz with xz_yz
    (S2) Sx*y=Sxy | S*x=Sx, Sxy*z=Sxyz, x*z=xz, y*z=yz, xz*yz=xz_yz
      --> merge Sxyz with xz_yz
    (S3) Sxy*z=Sxyz | Sx=S*x, Sx*y=Sxy, x*z=xz, y*z=yz, xz*yz=xz_yz
      --> insert Sxyz in xz_yz tree
    ...
    (S6) xz*yz=xz_yz | Sx=S*x, Sxy=Sx*y, Sxy*z=Sxyz, x*z=xz, y*z=yz
      --> insert xz_yz in Sxzy tree
  (Q1) each merger/insertion requires an axiom enforcement.  is there an
    efficient honduran sex-slave in the house?  anyone?  anyone?  is there
    an efficient way to do this other than enforcement queues?
    (A1) only {K2, S3, S6} need to be enforced upon creation, but all others
      need to be enforced upon insertion
(N8) maybe all random choice should be done through the floating-point
  calculated probs and the resulting choice tree.
  (N1) this has the disadvantage that, within one creation period, newly
    created nodes cannot be used to create newer nodes.
  (N2) it's also slower
  (N3) it also requires an additional choice tree (since the measure
    and its inverse require different choice trees)
(N9) on roots:
    node.app.IOM.getRoot() == node.app #used to represent ob
    node.MIO.getRoot() == node.lhs.MIO.getRoot()
    node.IMO.getRoot() == node.rhs.IMO.getRoot()
    node.IOM.getRoot() == node.app.IOM.retRoot()
    node.OIM.getRoot() == node.app.OIM.getRoot()
  these equations can be used to insert a node given {lhs,rhs,app}
(N10) need to smoke less pot while programming, make fewer mistakes.
(Q4) how to sort obs WRT prob?  can this be done in-place?  can it be done
  progressively, as a constantly updating thing?  each time the measure is
  recalculated?
(Q10) how to do the sub-heap insertion & removal?
  (N1) the heap is pre-ordered.  to do pre-order insertion:
"""
def insert(node,root):
  pos = root
  while pos:
    prevPos = pos
   #classify pos WRT node: {UL,UR,DL,DR}
    if pos.id | node.id > node.id:  vDiff = 'U'
    else:                           vDiff = 'D'
    if pos.id & node.id > node.id:  hdiff = 'L'
    else:                           hDiff = 'R'
    diff = vDiff + hdiff
    ...
"""
  (N2) of all the node operations {+,-,*,/,%,|,&,^,~,-}, one must
    correspond to vertical comparison and another to horiz. comparison.
    the heap ids look like:
"""
1
2   3
4 5 6 7
...
100 101 110 111
"""
    so < compares WRT the depth-then-horiz order.
  (N3) the most important points are that:
    (P1) descendents are always placed below their ancestors
    (P2) ancestors are always placed above their descendents
"""
def heap_insertStep(old,new):
  ...
"""
  (N4)
start with:
...
def heap_meet(x,y):
  ???
def heap_UL_DR_insert(UL,DR,x):
  assert UL is not DR
  assert UL is not x
  assert DR is not x
  assert UL.heap_above(DR)
  assert UL.heap_leftOf(DR)
  assert UL.heap_above(x)
  assert UL.heap_leftOf(x)
  meet = D.heap_meet(x)
  if meet is x:
    DR.heap_replaceBy(x)
    x.heap_U_insert(DR)
    ...
    if x.heap_above(DR):
      if x.heap_leftOf(meet): #==> DR.rightOf(meet)
